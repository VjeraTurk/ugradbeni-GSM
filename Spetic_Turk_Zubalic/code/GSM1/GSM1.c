 ...
 * programiranja ugradbenih racunalnih sustava na danom uredaju. U sredistu naseg  projekta je GSM/GPRS modul-SM5100B za mobilnu komunikaciju.
 * 
 *
...
/** PWM frequency for backlight color regulation*/
#define F_PWM           1000U 
/**prescalar*/
#define N               8
/**When counter reaches TOP value stored in OCR1A register it can generate overflow interrupt*/
#define TOP               (((F_CPU/F_PWM)/N)-1)
/** Value in OCR1B register */
...
/** Pin for regulating RED back light, PWM signal output pin*/
#define RED		PD4
/** Pin for regulating GREEN back light*/
#define GREEN	PD3
/** Pin for regulating BLUE back light*/
#define BLUE	PD2

/** Output pin for LED diode representing load or room lighting*/
#define LED	PA2
/**Output pin connected to GP 18 on GSM module for entering sleep mode (with high voltage)*/
#define SLEEP	PA3
...
/**
Buffer used for receiving data from USART
*/
 ...
 * 
 * In sleep mode, where GSM module sends sms via USART direct to avr upon receipt, 
 * variable first_data indicates that   
 */
...
/**
 * @brief Sends one character to USART
 * */
...
/**
 * @brief Sends string of characters to USART
 * */
 ...
 *
 * 4) If (CME and CMS )errors  and booting lines (which start with +SIND) are received, request is repeated (return 0) after awhile.
 * Global variable int sms_flag is set if UNREAD or READ message is found.
 *
 * @return 1 if there is no message with requested index (empty)or if there is
...
/**
 * @brief Function extracts phone number from rxBuffer, (after it is confirmed that rxBuffer containes a valid READ or UNREAD message in read_rxBuffer() function)
 * @param read
...
/** Contains last sender's phone number */
...
void LUX();
/**
 * @brief This function handles ad conversion
 * 
 * */
volatile uint8_t getLight(uint8_t);
/**
 * Indicates whether light is switched on or off*/
	volatile uint8_t upaljeno = 0;
/**Stores last phone number to which a question was sent (to turn the light on/off)*/
	volatile char from_number_lux[13];
/**
 * @brief This function sends at command for deleting sms of certain index. 
 * For indexes higher than 9, it also sends out index's tenner. 
 * (The highest index it can request is 99, but since SIM car memory can handle around 25 messages it is not required)
 * It than waits for GSM module to reply with OK, meaning sms was successfully deleted.
 *
 * @param index 
 * @param tenner
 */
 ...
 *
 * It is used after end of analyzing rxBuffer content to empty the buffer which contains received data.
 * Every new received answer from GSM is the only answer in rxBuffer.
...
/**@brief This function turns on red back light*/
void RED_light();
/**@brief This function turns on green back light*/
void GREEN_light();
/**@brief This function turns on blue back light*/
void BLUE_light();
/**@brief This function switches 6 back light colors, from red to violet*/
void rainbow();
 ...
 * Depending on SIM card type, it can contain 25 up to 32 sms messages in its memory. Other types of memory can contain more sms.
 *
...
*/
/**
 * @brief This function sends AT command to reboot GSM module
 * */
void reboot();
/**
 * @brief This function sends AT command to allow sleep mode. When sleep mode is allowed, 
 * high voltage on pin GP 18 puts GSM in sleep mode. 
 * */
void sleep_mode();
...
/**
 * @brief Function is called once the program is started. It enables text mode and then empties sim card sms storage.
 * Polling mode and sleep mode are 2 separate modes, this code is currently set for sleep mode.
 */
int main(void)
{
	init();
...
//////////Sleep mode, comment if using polling mode////////////:
...
///////while(0) polling mode while(1)sleep mode
	while(1){
...
/** Contains last sender's phone number (in sleep mode) */
